...
typealias FileNames = [String]
typealias FileContents = [String: String]

private let START_OF_TEXT = String(Unicode.Scalar(0002)!)
private let END_OF_TEXT = String(Unicode.Scalar(0003)!)
...
internal func save(contents: FileContents, to: String) {
    do {
        ...
        let data = buildContents(contents: contents)
...
private func buildContents(contents: FileContents) -> String {
    return contents.map {
        START_OF_TEXT +
        $0.0 + "\n" +
        $0.1 + "\n" +
